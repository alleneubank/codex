...
impl RuntimeKeymap {
    ...
    pub(crate) fn from_config(keymap: &TuiKeymap) -> Result<Self, String> {
        ...
        let cycle_permission_mode_default_is_shadowed =
            keymap.global.cycle_permission_mode.is_none()
                && (configured_main_surface_alias_is_used(keymap, "alt-p")
                    || configured_context_alias_is_used(&keymap.list, "alt-p")
                    || configured_context_alias_is_used(&keymap.approval, "alt-p")
                    || chords.bindings.iter().any(|binding| {
                        binding.action.context.overlaps(KeymapContext::Global)
                            && binding.chord.prefix.parts()
                                == key_hint::alt(KeyCode::Char('p')).parts()
                    }));
        ...
        let app = AppKeymap {
            ...
            cycle_permission_mode: if cycle_permission_mode_default_is_shadowed {
                Vec::new()
            } else {
                resolve_bindings(
        ...
        // Reasoning arrow and stash aliases are fallback defaults: existing
        // explicit bindings on the same input path remain authoritative.
...
mod tests {
    ...
    #[test]
    fn permission_mode_cycle_default_yields_to_existing_global_chord_prefix() {
        let mut keymap = TuiKeymap::default();
        keymap.global.open_transcript = Some(one("alt-p ctrl-t"));
