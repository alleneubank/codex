...
#[tokio::test]
async fn service_tier_toggle_refreshes_custom_status_line_payload() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let runner = FakeStatusLineRunner::new(vec![
        WorkspaceCommandOutput {
            exit_code: 0,
            stdout: "default status\n".to_string(),
            stderr: String::new(),
        },
        WorkspaceCommandOutput {
            exit_code: 0,
    ...
    crate::chatwidget::tests::set_fast_mode_test_catalog(&mut chat);
    chat.config.cwd = tempdir.path().to_path_buf().abs();
    chat.current_cwd = Some(tempdir.path().to_path_buf());
    chat.config.tui_custom_status_line = Some(CustomStatusLineConfig {
        kind: CustomStatusLineType::Command,
    ...
    let service_tiers = runner
        ...
        .map(|command| {
            ...
            .expect("payload should be valid JSON");
            payload.get("service_tier").cloned()
        })
        .collect::<Vec<_>>();
    assert_eq!(
        service_tiers,
        vec![
            Some(json!(SERVICE_TIER_DEFAULT_REQUEST_VALUE)),
            Some(json!(ServiceTier::Fast.request_value())),
        ]
    );
}
