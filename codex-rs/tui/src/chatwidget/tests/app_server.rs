...
#[tokio::test]
async fn live_app_server_server_overloaded_retry_keeps_turn_running() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
        ...
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                additional_details: Some(
                    "Selected model is at capacity. Please try again later.".to_string(),
                ),
            },
            will_retry: true,
            thread_id: "thread-1".to_string(),
    ...
    terminal
        ...
        .expect("draw retrying overload state");
    assert_chatwidget_snapshot!(
        "live_app_server_server_overloaded_retrying_state",
