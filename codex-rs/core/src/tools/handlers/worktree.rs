...
async fn enter_worktree(
    ...
    let entry = match (args.name, args.path) {
        ...
        (None, Some(path)) => {
            ...
            let same_repository_managed_worktree = if let Some(info) = candidate_info.as_ref()
                && managed_worktree_name(&info.common_dir, &info.repo_root).is_some()
            {
            ...
            if !same_repository_managed_worktree {
