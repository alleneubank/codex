...
pub(crate) fn build_permissions_update_item(
    ...
    let next_model_messages = next.model_info.model_messages.as_ref();
    let next_approval_messages =
        next_model_messages.and_then(|messages| messages.approvals.as_ref());
    let next_permission_messages =
        next_model_messages.and_then(|messages| messages.permissions.as_ref());
    let next_permissions = PermissionsInstructions::from_permission_profile(
        ...
        ApprovalPromptContext::new(
            ...
            next_approval_messages,
            next_permission_messages,
        ),
        exec_policy,
        #[allow(deprecated)]
        &next.cwd,
    ...
    let previous_permission_messages = previous_model
        .filter(|model| *model == next.model_info.slug.as_str())
        .and(next_permission_messages);
    let previous_permissions = PermissionsInstructions::from_permission_profile(
        ...
        ApprovalPromptContext::new(
            previous_approvals_reviewer,
            previous_approval_messages,
            previous_permission_messages,
        ),
        exec_policy,
        #[allow(deprecated)]
        &next.cwd,
