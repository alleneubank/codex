...
async fn responses_websocket_401_reload_account_change_returns_new_session_error() {
    ...
    let harness = websocket_harness_with_provider_auth_options(
        ...
        /*concurrent_reasoning_summaries_enabled*/ false,
        /*enabled_features*/ &[],
        Some(WebsocketHarnessAuth::Manager(auth_manager)),
    )
    .await;
    let mut client_session = harness.client.new_session();
    let prompt = prompt_with_input(vec![message_item("hello")]);
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    websocket_harness_with_provider_auth_options(
        ...
        concurrent_reasoning_summaries_enabled,
        enabled_features,
        auth.map(WebsocketHarnessAuth::Credentials),
    )
    .await
}

enum WebsocketHarnessAuth {
...
async fn websocket_harness_with_provider_auth_options(
    ...
    concurrent_reasoning_summaries_enabled: bool,
    enabled_features: &[Feature],
    auth: Option<WebsocketHarnessAuth>,
) -> WebsocketTestHarness {
    ...
    let client_auth_manager = auth.map(|auth| match auth {
        WebsocketHarnessAuth::Credentials(auth) => {
            codex_core::test_support::auth_manager_from_auth_with_home(
                auth,
                codex_home.path().to_path_buf(),
            )
        }
        WebsocketHarnessAuth::Manager(auth_manager) => auth_manager,
    });
