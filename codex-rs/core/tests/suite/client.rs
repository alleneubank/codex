...
struct StaticExternalChatgptAuth {
    ...
    refreshed: CodexAuth,
    resolve_gate: Option<Arc<ExternalAuthResolveGate>>,
}

#[derive(Default)]
struct ExternalAuthResolveGate {
    armed: std::sync::atomic::AtomicBool,
    started: tokio::sync::Notify,
    release: tokio::sync::Notify,
}

impl ExternalAuthResolveGate {
    fn arm(&self) {
...
impl ExternalAuth for StaticExternalChatgptAuth {
    fn resolve(&self) -> ExternalAuthFuture<'_, CodexAuth> {
        Box::pin(async {
            if let Some(resolve_gate) = &self.resolve_gate
                && resolve_gate
                    .armed
                    .swap(false, std::sync::atomic::Ordering::SeqCst)
            {
                resolve_gate.started.notify_one();
                resolve_gate.release.notified().await;
            }
            self.current
                .read()
                .map(|auth| auth.clone())
                .map_err(|_| std::io::Error::other("external auth test lock is poisoned"))
        })
    }
...
-> anyhow::Result<()> {
    ...
    .await;
    let resolve_gate = Arc::new(ExternalAuthResolveGate::default());
    auth_manager
        .set_external_auth(Arc::new(StaticExternalChatgptAuth {
            current: RwLock::new(CodexAuth::from_external_chatgpt_tokens(
                &account_a_token,
                "account-a",
                Some("pro"),
            )?),
            ...
            resolve_gate: Some(Arc::clone(&resolve_gate)),
        }))
        .await?;
    ...
    resolve_gate.arm();
    start_text_turn(&peer, "peer account race").await?;
    resolve_gate.wait_until_started().await;
    ...
    let error_event = wait_for_event(&peer, |ev| matches!(ev, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error_event else {
        unreachable!();
    };
    assert!(
        error.message.contains("Start a new session"),
        "unexpected peer-session race error message: {}",
        error.message
    );
    assert_eq!(error.codex_error_info, Some(CodexErrorInfo::Unauthorized));
    wait_for_event(&peer, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
