...
#[test]
fn plan_implementation_matcher_selects_only_plan_attention() {
    let request = NotificationRequest {
        session_id: codex_protocol::ThreadId::new(),
        ...
        model: "gpt-test".to_string(),
        permission_mode: "default".to_string(),
        notification_type: NotificationType::PlanImplementationRequest,
    };

    let selected = preview(
        &[
            handler(Some("user_input_request"), /*display_order*/ 0),
            handler(
                Some("plan_implementation_complete"),
                /*display_order*/ 1,
            ),
            handler(
                Some("plan_implementation_request"),
                /*display_order*/ 2,
            ),
        ],
        &request,
