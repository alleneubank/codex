...
if [[ "${FAKE_FILE_ARCH:-}" == arm64 ]]; then
  ...
  exit 0
elif [[ "${FAKE_FILE_ARCH:-}" == x86_64_dynamic ]]; then
...
dynamic_output=""
if dynamic_output="$(FAKE_FILE_ARCH=x86_64_dynamic "${real_bash}" \
  "${repo_root}/.github/scripts/verify-fork-release-bundle.sh" \
  x86_64-unknown-linux-musl \
  "${output_dir}/codex-x86_64-unknown-linux-musl-bundle.tar.zst" \
  "${expected_binary_version}" 2>&1)"; then
  echo "bundle verifier accepted a dynamically linked Linux bundle" >&2
  exit 1
fi
if [[ "${dynamic_output}" != *"static Linux x86_64 executable"* ]]; then
  echo "bundle verifier did not identify the Linux linkage mismatch: ${dynamic_output}" >&2
  exit 1
fi
