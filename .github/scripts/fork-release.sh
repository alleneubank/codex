...
existing artifacts, creates and pushes one annotated tag, and creates one
GitHub prerelease with the two platform bundles.
EOF
}
...
publish_release() {
  ...
  local_tag_sha="$(git -C "${repo_root}" rev-list -n 1 "${release_tag}" 2>/dev/null || true)"
  if [[ -n "${local_tag_sha}" && "${local_tag_sha}" != "${head_sha}" ]]; then
    die "Local tag ${release_tag} points to ${local_tag_sha}, expected ${head_sha}"
  fi
  remote_tag_sha="$(
    git -C "${repo_root}" ls-remote origin "refs/tags/${release_tag}" "refs/tags/${release_tag}^{}" |
      awk '
        NR == 1 { tag_sha = $1 }
        $2 ~ /\^\{\}$/ { peeled_sha = $1 }
        END {
  ...
  if [[ -n "${remote_tag_sha}" && "${remote_tag_sha}" != "${head_sha}" ]]; then
    die "Remote tag ${release_tag} points to ${remote_tag_sha}, expected ${head_sha}"
  fi
  ...
  if gh release view "${release_tag}" --repo alleneubank/codex >/dev/null 2>&1; then
