...
existing artifacts without rebuilding, publishes origin/main with
force-with-lease, creates and pushes one annotated tag, and creates one GitHub
prerelease with the two platform bundles.
...
Silicon macOS host and creates the local tag. After that succeeds, use
`publish --publish` to verify the existing bundles again and publish without
rebuilding. Adding `--publish` directly to the first `roll --run` remains the
one-step path when publication is already authorized.
EOF
}
...
git_root="${CODEX_FORK_RELEASE_GIT_ROOT:-${repo_root}}"
...
base_version="$("${version_script}" check)"
short_sha="$(git -C "${git_root}" rev-parse --short=9 HEAD)"
revision="$(git -C "${git_root}" rev-parse --short=12 HEAD)"
...
print_publish_command() {
  printf '  bash .github/scripts/fork-release.sh publish --publish --date %q --output-dir %q\n' \
    "${release_date}" "${output_dir}"
}
...
require_main_branch() {
  ...
  [[ "${branch}" == "main" ]] || die "Fork releases require local main"
}
...
local_tag_sha() {
  git -C "${git_root}" rev-list -n 1 "${release_tag}" 2>/dev/null || true
}

remote_tag_sha() {
  git -C "${git_root}" ls-remote origin \
...
publish_main_branch() {
  ...
  expected_remote_sha="$(
    git -C "${git_root}" rev-parse --verify refs/remotes/origin/main 2>/dev/null || true
  )"
  [[ -n "${expected_remote_sha}" ]] ||
    die "origin/main is not available locally; fetch it before publishing"
  git -C "${git_root}" push \
...
publish_verified_release() {
  ...
  if [[ -z "${remote_sha}" ]]; then
    git -C "${git_root}" push origin "refs/tags/${release_tag}:refs/tags/${release_tag}"
  fi
...
Run with --run to build and verify locally. After verification and publication
authorization, run the following command to publish the existing bundles
without rebuilding:
EOF
    print_publish_command
    cat <<EOF
If publication is already authorized, add --publish to the first --run instead.
EOF
    return
  fi
  ...
  if [[ "${publish_confirmed}" == true ]]; then
    ...
    echo "Prepared local release ${release_tag}"
    echo "Verify the existing bundles and publish without rebuilding with:"
    print_publish_command
  fi
}
