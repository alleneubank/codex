...
[[ "$(wc -l <"${curl_log}" | tr -d ' ')" == "3" ]] || fail "cache miss did not download exactly three inputs"
...
[[ "$(wc -l <"${curl_log}" | tr -d ' ')" == "3" ]] || fail "valid cache hit performed another download"
[[ "${after_mtimes}" == "${before_mtimes}" ]] || fail "valid cache hit changed cached input mtimes"
grep -Fqx "RUSTY_V8_ARCHIVE=${binding_dir}/${archive_name}" "${second_env}" || fail "cache hit exported the wrong archive path"
grep -Fqx "RUSTY_V8_SRC_BINDING_PATH=${binding_dir}/${binding_name}" "${second_env}" || fail "cache hit exported the wrong binding path"

printf 'corrupted archive\n' >"${binding_dir}/${archive_name}"
third_env="${test_root}/third.env"
run_setup "${third_env}"
[[ "$(wc -l <"${curl_log}" | tr -d ' ')" == "6" ]] || fail "invalid cache did not refresh all three inputs"
(
  cd "${binding_dir}"
  if command -v sha256sum >/dev/null 2>&1; then
