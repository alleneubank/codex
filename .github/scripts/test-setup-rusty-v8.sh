...
bad_fixture_dir="${test_root}/bad-fixtures"
workspace_root="${test_root}/workspace"
fake_bin="${test_root}/bin"
curl_log="${test_root}/curl.log"
runner_temp="${test_root}/runner-temp"
bad_curl_log="${test_root}/bad-curl.log"
mkdir -p "${fixture_dir}" "${bad_fixture_dir}" "${fake_bin}" "${runner_temp}"
mkdir -p "${workspace_root}/.github/scripts" "${workspace_root}/codex-rs" "${workspace_root}/third_party/v8"
for support_file in fork-python.sh rusty_v8_bazel.py rusty_v8_module_bazel.py run_bazel_with_buildbuddy.py; do
  cp "${repo_root}/.github/scripts/${support_file}" "${workspace_root}/.github/scripts/${support_file}"
done
cp "${repo_root}/codex-rs/Cargo.lock" "${workspace_root}/codex-rs/Cargo.lock"
...
trusted_checksums="${workspace_root}/third_party/v8/rusty_v8_${version//./_}_release_manifests.sha256"
printf '%s  %s\n' "$(hash_file "${fixture_dir}/${checksums_name}")" "${checksums_name}" >"${trusted_checksums}"
cp "${fixture_dir}/${archive_name}" "${bad_fixture_dir}/${archive_name}"
cp "${fixture_dir}/${binding_name}" "${bad_fixture_dir}/${binding_name}"
printf 'untrusted manifest\n' >"${bad_fixture_dir}/${checksums_name}"
...
run_setup() {
  local github_env="$1"
  : >"${github_env}"
  FAKE_CURL_SOURCE="${FAKE_CURL_SOURCE}" \
...
runner_temp="${test_root}/runner-temp-bad"
export FAKE_CURL_SOURCE="${bad_fixture_dir}"
export FAKE_CURL_LOG="${bad_curl_log}"
if run_setup "${test_root}/bad.env" 2>"${test_root}/bad-error.log"; then
  fail "untrusted checksum manifest was accepted"
fi
[[ "$(wc -l <"${bad_curl_log}" | tr -d ' ')" == "1" ]] ||
  fail "untrusted manifest downloaded payloads before verification"
grep -Fq "trusted checksum manifest mismatch" "${test_root}/bad-error.log" ||
  fail "untrusted manifest failure was not actionable"
